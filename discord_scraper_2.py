...
from config import DISCORD_TOKEN, DISCORD_CHANNEL_ID_2
...
class DiscordScraper2:
    def __init__(self, processed_ids_file="processed_messages_2.txt"):
        self.token = DISCORD_TOKEN
        self.channel_id = DISCORD_CHANNEL_ID_2
        self.session = None
    ...
    async def connect(self):
        ...
        try:
            async with self.session.get(
                ...
                logger.info(f"Monitoring channel ID (scraper 2): {self.channel_id}")
    ...
    async def get_new_messages(self):
        ...
        try:
            async with self.session.get(url, headers=headers) as response:
                ...
                if messages:
                    logger.info(f"Found {len(messages)} new messages from today (scraper 2)")
    ...
    async def close(self):
        if self.session:
            ...
            logger.info("Discord API session closed (scraper 2)")
